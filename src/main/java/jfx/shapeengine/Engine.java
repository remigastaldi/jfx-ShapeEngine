...
import jfx.shapeengine.shape.InteractivePolygon;
import jfx.shapeengine.shape.InteractiveRectangle;
import jfx.shapeengine.shape.InteractiveShape;
...
public class Engine {
  ...
  public Engine(StackPane stackPane, double width, double height) {
    ...
    content.setStyle("-fx-background-color: #212226");
    ...
    scrollPane.setStyle("-fx-background-color: #212226");
    ...
    content.setOnScroll(evt -> {
      if (evt.isControlDown()) {
        ...
        if (delta == 0d)
          return;
        final double zoomFactor = delta > 0 ? 1.2 : 1 / 1.2;
        zoom(zoomFactor);
      }
    });
